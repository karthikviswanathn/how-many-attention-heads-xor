...
&=
\left(\gamma+\sum_{i=1}^{n}\rho_i\right)
+
\sum_{i=1}^{n}\rho_i(\alpha-1)x_i.
\end{aligned}
$$
...
The monotone DNF construction in [23_monotone_dnf_upper_bound.md](23_monotone_dnf_upper_bound.md) uses exactly this orientation freedom: each conjunction atom chooses a negative-orientation denominator, so missing any required positive literal makes the denominator large and the atom contribution tiny.
