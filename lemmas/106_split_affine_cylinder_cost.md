...
Given strict affine-cylinder scores $S_0,S_1$ for the two cofactors, written as in [105_affine_cylinder_cofactor_interpolation.md](105_affine_cylinder_cofactor_interpolation.md), define their split interpolation cost by
...
For every split coordinate $j$, the two cofactors have strict affine-cylinder scores because $\operatorname{actc}$ is finite for every Boolean function by [97_affine_cylinder_threshold_cost.md](97_affine_cylinder_threshold_cost.md). Hence the minimum defining $\operatorname{sactc}_{j}(f)$ is over a nonempty set of nonnegative integers.

Fix a split coordinate $j$ and strict affine-cylinder cofactor scores $S_0,S_1$. Lemma [105_affine_cylinder_cofactor_interpolation.md](105_affine_cylinder_cofactor_interpolation.md) gives
...
The affine-cylinder threshold-cost theorem [97_affine_cylinder_threshold_cost.md](97_affine_cylinder_threshold_cost.md) gives
...
If $\operatorname{sactc}(f)\leq2$, then $\operatorname{actc}(f)\leq2$. The low affine-cylinder exactness theorem [103_low_affine_cylinder_cost_exactness.md](103_low_affine_cylinder_cost_exactness.md) now gives the displayed constant, nonconstant LTF, or two-head split. $\blacksquare$
