...
For the degree version, apply the shared-statistic degree sandwich [153_shared_statistic_degree_sandwich.md](153_shared_statistic_degree_sandwich.md). The constant background slices have degree $0$, so only the addresses in $A$ contribute to the upper bound, and the lower bound is the maximum threshold degree among the addressed slices. $\blacksquare$
