...
$$
\sum_{j=1}^{k}\rho_j a^{(q)}_j
<
\sum_{j=1}^{k}\rho_j a^{(q+1)}_j.
$$
...
$$
F_{a^{(0)}}(\tau_0),\ldots,F_{a^{(0)}}(\tau_{M-1}),
F_{a^{(1)}}(\tau_0),\ldots,F_{a^{(1)}}(\tau_{M-1}),
\ldots,
F_{a^{(2^k-1)}}(\tau_0),\ldots,F_{a^{(2^k-1)}}(\tau_{M-1}).
$$

Its sign changes are exactly the within-slice sign changes, totaling $\sum_a C_a$, plus the boundary jumps counted by $J_{\rho}$. The positive-projection sign-change upper bound [07_positive_projection_sign_changes.md](../01_foundations_and_normal_form/07_positive_projection_sign_changes.md) applied to $s$ gives
