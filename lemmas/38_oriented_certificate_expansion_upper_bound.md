...
can be approximated uniformly by one head by [35_ptf_sparsity_upper_bound.md](35_ptf_sparsity_upper_bound.md). By global bit-flip invariance from [22_restrictions_and_sign_rank.md](22_restrictions_and_sign_rank.md), the same is true for a signed negative monomial
...
If $\mathcal{C}_0$ is a $0$-certificate cover for $f$, then it is a $1$-certificate cover for $1-f$. Applying the first part to $1-f$ and using complement invariance from [22_restrictions_and_sign_rank.md](22_restrictions_and_sign_rank.md) proves the $0$-certificate bound. $\blacksquare$

## Consequence

The mixed-literal DNF/CNF literal-expansion theorem [36_dnf_cnf_literal_expansion_upper_bound.md](36_dnf_cnf_literal_expansion_upper_bound.md) is strengthened to the local bound
...
The decision-tree leaf-profile theorem [37_decision_tree_upper_bound.md](37_decision_tree_upper_bound.md) is strengthened similarly. If a decision tree has depth at most $d$, then every leaf satisfies
