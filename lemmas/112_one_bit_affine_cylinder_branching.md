...
Now apply the split affine-cylinder interpolation invariant [106_split_affine_cylinder_cost.md](106_split_affine_cylinder_cost.md) to the split coordinate $z$ and these two cofactor scores.
...
If the right-hand side is at most $2$, then $\operatorname{sactc}(F)\leq2$. The exact case split follows from [106_split_affine_cylinder_cost.md](106_split_affine_cylinder_cost.md). $\blacksquare$
...
Thus this lemma recovers the one-bit sparse-PTF branching bound [74_one_bit_sparse_ptf_branching.md](74_one_bit_sparse_ptf_branching.md) after replacing the nonlinear monomial support by positive cylinders.
