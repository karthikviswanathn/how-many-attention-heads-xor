...
As in [44_intersection_profile_bounds.md](44_intersection_profile_bounds.md), $R$ has degree $C(F)$ and satisfies
...
\mathrm{SUB}_m(x,y)
:=
\mathbf{1}[x_i\leq y_i\text{ for every }i],
$$
...
$$
\mathrm{SUB}_m(x,y)=\mathbf{1}[D(x,y)=0],
\qquad
\mathrm{NCON}_m(x,y)=\mathbf{1}[D(x,y)\geq1].
$$
...
For $m=2$, this is sharpened in [51_two_pair_containment_exact.md](51_two_pair_containment_exact.md) to

$$
H^{*}(\mathrm{SUB}_2)=H^{*}(\mathrm{NCON}_2)=2.
$$

For $m=3$, [53_three_pair_endpoint_exact.md](53_three_pair_endpoint_exact.md) gives
