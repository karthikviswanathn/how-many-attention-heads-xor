...
First, if $q=\chi$ or $q=-\chi$, then $f$ is parity or its complement. The parity threshold-degree lemma from [xor_n_bits.md](xor_n_bits.md) gives
...
Threshold degree alone cannot prove superlinear head lower bounds. In particular, the counting lower bound in [20_counting_lower_bound.md](20_counting_lower_bound.md) is a genuinely different obstruction: it shows that typical functions need exponentially many heads, even though every threshold-degree lower bound is at most $n$.
