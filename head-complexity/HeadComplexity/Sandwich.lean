...
where `deg±` (= `thresholdDeg`) is the threshold degree (lower bound, Lemma 6/7)
and `M = |Im(t)|` is the image size of any positive weighted sum `t(x) = ∑ λ_i x_i`
through which `f` factors (upper bound, Lemma 9). Closing this gap — pinning `H*`
...
for **all** `f`, the minimal-head form of Lemma 6) and assembles the sandwich.
-/
...
`deg±(f) ≤ H*(f)`. This is Lemma 6 evaluated at the minimal head count; for parity
Lemma 7 makes it tight (`deg±(parity) = n = H*(parity)`), and more generally it is
...
theorem thresholdDeg_le_HStarN_le_image_card (f : (Fin n → Bool) → Bool)
  ...
  ⟨thresholdDeg_le_HStarN f, HStarN_le_weighted lam hlam f G hf⟩
