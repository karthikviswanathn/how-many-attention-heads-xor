...
/-- **Lemma 1.** A one-head numerator, restricted to two coordinates, splits
additively. -/
alias lemma1_additive_split := NHead.numerator_additive_split

/-- **Lemma 2.** Antipode identity for the restricted one-head numerator. -/
alias lemma2_antipode := NHead.restricted_numerator_antipode

/-- **Lemma 3.** A checkerboard restriction forces `H* ≥ 2`. -/
alias lemma3_checkerboard := parity_restriction_HStarN_ge_two
