...
theorem softmax3_denom_pos (s : Fin 3 → ℝ) : 0 < ∑ j, Real.exp (s j) := by
...
theorem softmax3_pos (s : Fin 3 → ℝ) (i : Fin 3) : 0 < softmax3 s i :=
  div_pos (Real.exp_pos _) (softmax3_denom_pos s)

/-- Softmax weights are nonnegative. -/
theorem softmax3_nonneg (s : Fin 3 → ℝ) (i : Fin 3) : 0 ≤ softmax3 s i :=
  (softmax3_pos s i).le

/-- Softmax weights sum to one. -/
theorem softmax3_sum_one (s : Fin 3 → ℝ) : ∑ i, softmax3 s i = 1 := by
