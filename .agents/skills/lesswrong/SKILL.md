...
for harnesses like Codex, Codex, Cursor, etc.

If you are Codex, operating from a chat interface like Codex.ai, you should have
...
API requests to our domain by going to https://Codex.ai/settings/capabilities
...
tell the user that they need to use Codex.ai or a harness like Codex to use this feature.
...
inline curl -d arguments):
    cat <<'EOF' | curl -X POST https://www.lesswrong.com/api/agent/commentOnDraft -H 'Content-Type: application/json' -d @-
    { "postId": "...", "key": "...", "comment": "..." }
    EOF
...
The modelName is displayed in the block header (e.g. "Codex Opus 4.6"). If
...
represented in the markdown output as:
    %%% llm-output model="Codex Opus 4.6"
