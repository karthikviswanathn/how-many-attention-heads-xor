---
title: "How many attention heads do you need to do XOR?"
author: "Karthik Viswanathan"
date: "2 April 2026"
subtitle: "Originally published on [LessWrong](https://www.lesswrong.com/posts/T66BKwSufh5SfiPHm/how-many-attention-heads-do-you-need-to-do-xor-3)"
header-includes:
  - \usepackage{float}
...
XOR is one of the most widely used Boolean functions that is not linearly separable, making it a [classic test](https://en.wikipedia.org/wiki/Perceptrons_\(book\)#The_XOR_affair) of computational expressivity, from perceptrons to modern transformer circuits. Understanding how many attention heads are needed to compute it tells us something fundamental about what individual components of a transformer can and cannot represent.

Introduction
============

![**Computing XOR using an attention head.** Two input bits $(a,b)$ and the query token `=` are embedded and processed by a single attention head with a skip connection. Is the residual stream of the query token $h_=(a,b)$ linearly separable into XOR classes?](images/xor-architecture.png){width=80%}
...
![**One attention head cannot compute XOR, but two can.** With a single head, the line segments connecting same-class points always intersect, ruling out linear separability. With two heads, each head's segments still intersect individually, but linear separability emerges in the sum of their contributions.](images/xor-geometry.png){width=90%}
...
$$
P \;\in\; [z_=(0,0),\, z_=(1,1)] \;\cap\; [z_=(0,1),\, z_=(1,0)].
$$
...
$$
x_a=e_a+\text{pos}_a,\qquad x_b=e_b+\text{pos}_b,\qquad x_==e_{=}+\text{pos}_=.
$$
...
$$
h_=(a,b)= x_=+\sum_{j=1}^3 \alpha_j(a,b)\,W_Vx_j
$$

where $\alpha_j(a,b)$ is the attention weight from the $j^{\text{th}}$ key to the `=` token given as:

$$
\alpha_j(a,b)=\dfrac{\exp\bigl(x_=^\top W_Q^\top W_K x_j\bigr)}{\sum_{k=1}^3 \exp\bigl(x_=^\top W_Q^\top W_K x_k\bigr)}
$$
...
$$
z_=(a,b) := p_a\, v_a + p_b\, v_b + p_=\, v_=, \qquad p_a + p_b + p_= = 1,\quad p_j > 0.
$$
...
$$
\sigma_a := \exp(x_=^\top W_Q^\top W_K x_a), \quad \sigma_b := \exp(x_=^\top W_Q^\top W_K x_b), \quad \sigma_= := \exp(x_=^\top W_Q^\top W_K x_=)
$$

resulting in the following attention weights:

$$
p_j = \dfrac{\sigma_j}{\sigma_a + \sigma_b + \sigma_=}.
$$

So we can equivalently write the attention update directly in terms of the $\sigma$'s:

$$
z_=(a,b) = \dfrac{\sigma_a\, v_a + \sigma_b\, v_b + \sigma_=\, v_=}{\sigma_a + \sigma_b + \sigma_=}.
$$

We now ask: can a hyperplane separate the four attention outputs $\{z_=(a,b)\}_{a,b \in \{0,1\}}$ into the XOR classes?

$$
w^\top z_=(a,b) > \tau \quad\Longleftrightarrow\quad a \oplus b = 1.
$$
...
$$
z_=(a,b)=\dfrac{\sigma_a v_a+\sigma_b v_b+\sigma_= v_=}{\sigma_a+\sigma_b+\sigma_=}:= \dfrac{N(a,b)}{D(a,b)}.
$$
...
$$
N(a,b) = \underbrace{\sigma_a v_a}_{a\text{-only}}+\underbrace{\sigma_b v_b}_{b\text{-only}}+\underbrace{\sigma_= v_=}_{\text{const}}, \qquad D(a,b)=\underbrace{\sigma_a}_{a\text{-only}}+\underbrace{\sigma_b}_{b\text{-only}}+\underbrace{\sigma_=}_{\text{const}}.
$$
...
$$
N(0,0)+N(1,1) = N(0,1)+N(1,0) = \mathcal{N},
$$

$$
D(0,0)+D(1,1) = D(0,1)+D(1,0) = \mathcal{D}.
$$

Line segments connecting the same class intersect
-------------------------------------------------

![**The geometric obstruction.** The segment connecting the XOR-negative outputs $[z_=(0,0), z_=(1,1)]$ (blue) always crosses the segment connecting the XOR-positive outputs $[z_=(0,1), z_=(1,0)]$ (orange) at a point $P$. Any hyperplane $w^\top z = \tau$ must place $P$ on both sides simultaneously, making linear separation impossible.](images/xor-geometry-left.png){width=70%}
...
$$
\begin{aligned} \mathcal{N} = D(0,0)\, z_=(0,0) + D(1,1)\, z_=(1,1) \\ = D(0,1)\, z_=(0,1) + D(1,0)\, z_=(1,0). \end{aligned}
$$

Dividing both expressions by $\mathcal{D} = D(0,0) + D(1,1) = D(0,1) + D(1, 0)$, we obtain

$$
\begin{aligned} P &:= \dfrac{\mathcal{N}}{\mathcal{D}} \\ &= \dfrac{D(0,0) \,z_=(0,0) + D(1,1) \,z_=(1,1)}{D(0,0) + D(1,1)} \\ &= \dfrac{D(0,1) \,z_=(0,1) + D(1,0) \,z_=(1,0)}{D(0,1) + D(1, 0)}. \end{aligned}
$$
...
$$
z_=(a,b)=y_=^{(0)}(a,b)+y_=^{(1)}(a,b),
$$

where

$$
y_=^{(r)}(a,b):=\sum_{j=1}^3 \alpha_j^{(r)}(a,b)\,W_V^{(r)}x_j.
$$
...
$$
e_0=(1,0,0),\qquad e_1=(0,1,0),\qquad e_{=}=(0,0,1).
$$
